...
export default function ScenarioForm({
  ...
  const updateCurrentStock = (branch: string, value: number) => {
    onScenarioChange({
      ...scenario,
      current_stock: { ...scenario.current_stock, [branch]: Math.max(0, value) },
    })
  }
  ...
  const updateTransportCost = (from: string, to: string, value: number) => {
    if (from === to) return
    const transport_costs = { ...scenario.transport_costs }
    transport_costs[from] = { ...transport_costs[from], [to]: Math.max(0, value) }
  ...
  const validate = (): string | null => {
    ...
    for (const b of scenario.branches) {
      ...
      if (cur === undefined || cur < 0) return `Stoc curent invalid pentru ${b}.`
      if (min === undefined || min < 0) return `Stoc minim invalid pentru ${b}.`
      if (tinta === undefined || tinta < 0) return `Stoc țintă invalid pentru ${b}.`
      if (tinta < min) return `Stoc țintă pentru ${b} trebuie să fie >= stoc minim.`
    }
  ...
  return (
    <form className="scenario-form" onSubmit={handleSubmit}>
      ...
      <section>
        <h2>Stocuri</h2>
        <p className="hint">
          Stoc țintă = nivelul dorit după redistribuire (≥ minim). Transportăm până la țintă, nu doar până la minim, astfel încât la prima vânzare filiala să nu reintre în deficit.
        </p>
              ...
              <tr key={b}>
                ...
                <td>
                  <input
                    type="number"
                    min={0}
                    value={scenario.target_stock[b] ?? scenario.min_stock[b] ?? 0}
                    onChange={(e) =>
      ...
      <section>
        ...
        >
          {showCostMatrix ? 'Ascunde' : 'Editează'} matricea de costuri transport
        </button>
        {showCostMatrix && (
                      ...
                      <td key={to}>
                        {from === to ? (
                          ...
                          <input
                            ...
                            step={0.01}
                            value={
                              scenario.transport_costs[from]?.[to] ?? ''
                            }
                            onChange={(e) =>
                              updateTransportCost(
                                from,
